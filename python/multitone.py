...
def generate_multitone(samplerate, duration_sec, active_ratio, f0, f1, scale,
                       outfile):
    ...
    while freq <= f1:
        signal_array.append(generate_sin(active_duration_sec, freq,
                                         samplerate, scale))
        freq *= 2
...
def get_options(argv):
    ...
    parser.add_argument(
        ...
        help='Zero verbosity',)
    parser.add_argument(
        '--samplerate', action='store', type=int,
        dest='samplerate', default=default_values['samplerate'],
        metavar='SAMPLERATE',
        help='use SAMPLERATE samplerate',)
    parser.add_argument(
        '--duration-sec', action='store', type=float,
        dest='duration_sec', default=default_values['duration_sec'],
        metavar='DURATION_SEC',
        help='use DURATION_SEC duration_sec',)
    parser.add_argument(
        '--active-ratio', action='store', type=float,
        dest='active_ratio', default=default_values['active_ratio'],
        metavar='ACTIVE-RATIO',
        help='use active_ratio',)
    parser.add_argument(
        '--f0', action='store', type=int,
        dest='f0', default=default_values['f0'],
        metavar='F0',
        help='use F0 for start chirp frequency',)
    parser.add_argument(
        '--f1', action='store', type=int,
        dest='f1', default=default_values['f1'],
        metavar='F1',
        help='use F1 for end chirp frequency',)
    parser.add_argument(
        '--scale', action='store', type=float,
        dest='scale', default=default_values['scale'],
        metavar='SCALE',
        help='use scale',)
    parser.add_argument(
        '--signal', action='store', type=str,
        dest='signal', default=default_values['signal'],
        choices=SIGNAL_CHOICES,
        metavar='[%s]' % (' | '.join(SIGNAL_CHOICES,)),
        help='signal arg',)
    parser.add_argument(
        'outfile', type=str,
...
def main(argv):
    ...
    if options.signal == 'multitone':
        generate_multitone(options.samplerate, options.duration_sec,
                           options.active_ratio, options.f0, options.f1,
                           options.scale, options.outfile)
    else:
